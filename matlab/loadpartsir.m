function [image, head, descrip, iaopt]=loadpartsir(filename,xll,yll,xur,yur)
%
%   [image head descrip iaopt]=loadpartsir('filename',xll,yll,xur,yur)
%
% load part of a sirf image into matlab.  Also sets global variables
...
% xll,yll:    lower left corner of selected portion of image (normal convention)
% xur,yur:    lower left corner of selected portion of image (normal convention)
%
...
if xll == 1 & yll == 1 & xur == head(1) & yur == head(2)  % read entire image
...
%    disp(['Read integer data: ' num2str(head(1)) ' x ' num2str(head(2))]);
  ...
  im_in=zeros([xur-xll+1,yur-yll+1]);
  if idatatype == 1 
    for j=yll:yur
      offset=2*((j-1)*head(1)+(xll-1))+512*nhead;
      fseek(fid,offset,-1);
      im_in(:,j-yll+1)=fread(fid,xur-xll+1,'schar');
    end
  elseif idatatype == 4
    for j=yll:yur
      offset=2*((j-1)*head(1)+(xll-1))+512*nhead;
      fseek(fid,offset,-1);
      im_in(:,j-yll+1)=fread(fid,xur-xll+1,'float');
    end
  else
    for j=yll:yur
      offset=2*((j-1)*head(1)+(xll-1))+512*nhead;
      fseek(fid,offset,-1);
      im_in(:,j-yll+1)=fread(fid,xur-xll+1,'short');
    end
  end;
  image=rot90(s*im_in+soff+ioff);   % scale data to floating point and
  ...
  % alter projection information so SIR transformation code can be used
  ...
  % save new transformation variables
  
  head(1)=xur-xll+1;     % nsx
  head(2)=yur-yll+1;     % nsy
  head(8)=a0;
  head(9)=b0;
