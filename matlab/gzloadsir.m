function [img, head, descrip, iaopt] = gzloadsir(file)
%
%   [img head descrip iaopt]=gzloadsir(filename)
%
% loads a BYU .SIR file.  The file name can contain a .gz extension
% to indicated that it is gzipped.  See loadsir.m
...
if status ~= 0      % input file is gzipped
  ...
  [img head descrip iaopt]= loadsir([tmpdir '/' fname]);
  ...
  % load the file
  [img head descrip iaopt] = loadsir(file); 
end
