function viewsir_map1(action,filein,cia_filein)
%
% viewsir_map1(input_file_name)
%
...
%
% viewsir_map1(action,file_name_in,cia_filein)
%
% matlab utility for viewing sir images with a land mask
%
...
if strcmp(action,'initialize'),
    ...
    if nargin > 1
      ...
      filename='/auto/cers0/long/sir/data/greeni.sir';
    end;

    if nargin > 2
      map_filename=cia_filein;
    else
      map_filename='/auto/cers0/long/cia/antarctica.dat';
    end;
 ...
 % The view popup button
   ...
   %====================================
    ...
    btnNumber=2;  
    labelStr='Rescale';
    callbackStr='viewsir_map1(''scale'')';
    yLabelPos=top-(btnNumber-1)*(btnHt+labelHt+spacing);
    btnPos=[left yLabelPos-labelHt-btnHt-btnOffset btnWid btnHt];
    infoHndl=uicontrol( ...
        'Style','push', ...
   ...
   %====================================
    ...
    btnNumber=3;  
    labelStr='Point';
    callbackStr='viewsir_map1(''sir_locate1'')';
    yLabelPos=top-(btnNumber-1)*(btnHt+labelHt+spacing);
    btnPos=[left yLabelPos-labelHt-btnHt-btnOffset btnWid btnHt];
    infoHndl=uicontrol( ...
        'Style','push', ...
   ...
   %====================================
    ...
    figure(figNumber);
    shwimg1(filename);  % display image
    colormap('gray');   % default grayscale color table
    
    if nargin > 2       % add outline map if supplied
      plot_mapfile(map_filename);
    end
...
function plot_mapfile(mapfilename)

    global sir_head_viewsir;
    global sir_im_viewsir;

    disp(['Loading map file: ' mapfilename]);
    line=['load ' mapfilename];  % read map file
    eval(line);
    
    i1=1;
    i2=length(mapfilename);
    k=findstr('/',mapfilename);  % strip off path
    kk=length(k);
    if kk > 0
      i1=k(kk)+1;
    end
    k=findstr('.',mapfilename);   % strip off extension
    kk=length(k);
    if kk > 0
      i2=k(kk)-1;
    end
    name=mapfilename(i1:i2);
    line=['map=' name ';'];
%    disp(['map command: ' line])
    eval(line);                  % store map in known variable
    line=['clear ' name];
    ...
    for k=1:n-1
      ...
      [x, y] = latlon2pix(lon,lat,sir_head_viewsir);
      l=length(x);
      if l > 1
	%   for i=1:l-1
	...
	%     A(p)=+10;
	%   end
	plot(x-0.5,y+0.5);  % offsets compensate for matlab's use of
                            % center of pixel points
      end
    end
