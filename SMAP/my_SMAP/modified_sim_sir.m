...
axis off
colorbar
title('Simulated Ancillary Data')

ancil_unroll = reshape(ancil, nsize, 1);

b_val = (b_val.^2)/500 + ancil_unroll;
...
a_val = (a_val.^2)/500 + ancil_unroll;
b_val = zeros(nsize, 1);
...
for its = 0:nits - 1
    ...
    [a_val, a_temp, tot, sx, sx2, total] = get_modified_updates(pow', ang, count, pointer, aresp1, a_val, sx, sx2, a_temp, tot, ancil_unroll); 
...
a_val2 = load('a_val.mat');
a_val2 = (a_val2.a_val.^2)/500 + ancil_unroll;
temp4 = reshape(a_val2, [nsx, nsy]);
figure(3)
imagesc(temp4)
colormap(gray)
title('Separated rSIR')
axis off
colorbar
...
figure(4)
imagesc(diff)
title('Difference between combined rSIR and separated rSIR')
axis off 
colorbar
