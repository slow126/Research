function [ dcArray ] = sm2dc_v2( smArray, clayfrac )
...
dcArray=nan(size(smArray));
        ...
        less_ind = (smArray <= mvt);
        nm(less_ind) = nd(less_ind)+(nb(less_ind)-1).*smArray(less_ind);
        km(less_ind) = kd(less_ind)+kb(less_ind).*smArray(less_ind);
        real_part(less_ind) = nm(less_ind).^2-km(less_ind).^2;
        im_part(less_ind) = 2.*nm(less_ind).*km(less_ind);

        great_ind = (smArray > mvt);
        nm(great_ind) = nd(great_ind)+(nb(great_ind)-1).*mvt(great_ind) + (nu(great_ind)-1).*(smArray(great_ind)-mvt(great_ind));
        km(great_ind) = kd(great_ind)+kb(great_ind).*mvt(great_ind) + ku(great_ind).*(smArray(great_ind)-mvt(great_ind));
        real_part(great_ind) = nm(great_ind).^2-km(great_ind).^2;
        im_part(great_ind) = 2.*nm(great_ind).*km(great_ind);
        dcArray=real_part+1i*im_part;
