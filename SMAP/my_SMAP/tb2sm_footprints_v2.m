function [sm_meas, sm_fill_array, sm_response_array, sm_footprint] = tb2sm_footprints_v2(tbav, fill_array, resp_array,  year, day, res, albav, incav, qualav, clayf, vopav, rghav, smav, vwcav, tempav, wfracav)
...
for i = 1:5000 %length(tbav)
    ...
    idx = find(~isnan(tb_map));
            ...
            emis=tb./tb_temp; %calculate the rough emissivity
...
%         moisture_map = flipud(moisture_map);
        ...
        moisture_map(idx) = possible_mois(dif_ind);
...
%             j = j + 1;
    ...
    if ~isempty(find(footprint,1))
        ...
        footprint = footprint(mask) ./ sm_meas(m);
