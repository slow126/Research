...
extradata=0;    % Loads some extra information from the HDF5

if extradata
    run('data_load3.m'); % Load data from an HDF5 file - edit to match desired polarity and day
else 
    run('data_load2.m'); % Load data from an HDF5 file - edit to match desired polarity and day
end

run('clay_load.m'); % Load the clay fraction data
...
for picx=1:nsx1
    for picy=1:nsy1
        ...
        if(isnan(tb) || isnan(localclay) || isnan(localtemp) || isnan(vwc) ...
                || isnan(albedo) || isnan(vegop) || isnan(inc) || isnan(roughness) || (quality ~= 0 && quality ~= 8) ) %Not sure on the quality flag
            continue; %skip if we don't have all necessary info
        end
...
goodind=find(~((smav > 1.05*moisture_map + 0.08) | (smav < 0.75*moisture_map-.05)));
