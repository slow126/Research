...
%         scat_temp(isnan(scat_temp)) = [];
        ...
        scat_temp = db2mag(reshape(sigma_hC(i,j,:),1,z));
...
% 
%         my_Beta_vars = polyfit(rad_temp,scat_temp,1);
%         B_C(i,j) = my_Beta_vars(1,1);
           B_C(i,j) = cond_expect(reshape(rad_hC(i,j,:),1,z),reshape(sigma_hC(i,j,:),1,z),sigma_hC(i,j,1));
    end
end
...
for i = 1:z-1
    for j = 1:size(sigma_hM,1)-1
        for h = 1:size(sigma_hM,2)-1
            rad_hM(j,h,i) = rad_hC(floor(j/2)+1,floor(h/2)+1,i)...
                + B_C(floor(j/2)+1,floor(h/2)+1) .* ((sigma_hM(j,h,i)...
                - sigma_hC(floor(j/2)+1,floor(h/2)+1,i)) + gamma(j,h,i)...
                .* (sigma_vC(floor(j/2)+1,floor(h/2)+1,i) - sigma_vM(j,h,i)));
        end
    end
end
