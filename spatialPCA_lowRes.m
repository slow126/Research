...
lowRes_PCA_img = zeros(size(rad_h,1),size(rad_h,2));
for i = 1:10:y /(reduction_factor/divisor)
    for j = 1:10:x /(reduction_factor/divisor)
        ...
        if(isempty(COEFF))
            ...
            lowRes_PCA_img(i:(i+index.plus_y(i)-index.minus_y(i)+1),j:(j+index.plus_x(j)-index.minus_x(j)+1)) = pca1(center_of_windowY,center_of_windowX);  
        end
...
save('lowResPca.mat','lowRes_PCA_img');